      ...
      if (location2.attrs) {
        for (key2 in location2.attrs) {
          if (own.call(location2.attrs, key2)) {
            properties2[find(state.schema, key2).property] = position(
...
function element2(state, unsafe) {
  ...
  if (name && name !== "*" && (!state.schema.tagNames || state.schema.tagNames.includes(name))) {
    safeElement = true;
    if (state.schema.ancestors && own2.call(state.schema.ancestors, name)) {
...
function properties(state, properties2) {
  ...
  const specific = attributes && own2.call(attributes, tagName) ? attributes[tagName] : void 0;
  const defaults = attributes && own2.call(attributes, "*") ? attributes["*"] : void 0;
  const properties_ = (
  ...
  if (required && own2.call(required, tagName)) {
    const properties3 = required[tagName];
    for (key2 in properties3) {
      if (own2.call(properties3, key2) && !own2.call(result, key2)) {
...
function safeProtocol(state, key2, value) {
  const protocols = state.schema.protocols && own2.call(state.schema.protocols, key2) ? state.schema.protocols[key2] : void 0;
...
function omission(handlers2) {
  ...
  function omit(node2, index2, parent) {
    return own5.call(handlers2, node2.tagName) && handlers2[node2.tagName](node2, index2, parent);
  }
}
...
var handle = zwitch("type", {
  invalid,
  unknown,
  handlers: { comment: comment2, doctype: doctype2, element: element3, raw, root: root2, text: text2 }
});
...
function decodeNamedCharacterReference(value) {
  return own6.call(characterEntities, value) ? characterEntities[value] : false;
}
...
function createState(tree, options) {
  ...
  function one4(node2, parent) {
    ...
    if (own8.call(state.handlers, type) && handle3) {
...
var Processor = class _Processor extends CallableInstance {
  ...
  data(key2, value) {
    if (typeof key2 === "string") {
      ...
      return own9.call(this.namespace, key2) && this.namespace[key2] || void 0;
    }
    if (key2) {
      assertUnfrozen("data", this.frozen);
      this.namespace = key2;
...
var joinSource = (source) => Array.isArray(source) ? source.join("") : source ?? "";
var escapeHtml = (text8) => text8.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#x27;");
var NotebookEmbedding = (userOpts) => {
  const opts = { ...defaultOptions, ...userOpts };
  const markdownToHtml = async (markdown) => {
    try {
      const result = await unified().use(remarkParse).use(remarkGfm).use(remarkMath).use(remarkRehype, { allowDangerousHtml: true }).use(rehypeStringify, { allowDangerousHtml: true }).process(markdown);
  ...
  const sanitizeFragment = (html7) => {
    const tree = sanitize(fromHtml(html7, { fragment: true }), outputSchema);
  ...
  const formatOutput = (output) => {
    ...
    if (output.data["text/html"]) {
      content3 += `<div class="notebook-html-output">${sanitizeFragment(joinSource(output.data["text/html"]))}</div>`;
    } else if (output.data["text/plain"]) {
      content3 += `<div class="notebook-text-output"><pre>${escapeHtml(joinSource(output.data["text/plain"]))}</pre></div>`;
    }
    if (output.data["image/png"]) {
      const b64 = joinSource(output.data["image/png"]).replace(/\s/g, "");
      content3 += `<div class="notebook-image-output"><img src="data:image/png;base64,${b64}" alt="Notebook output" loading="lazy" /></div>`;
    }
    return content3;
  };
  const cellToHtml = async (cell, index2, language) => {
    let content3 = "";
    if (cell.cell_type === "markdown") {
      content3 = `<div class="notebook-markdown-cell">${await markdownToHtml(joinSource(cell.source))}</div>`;
    } else if (cell.cell_type === "code") {
      const count = cell.execution_count ?? null;
      const label = count === null ? "In [ ]:" : `In [${count}]:`;
      content3 = `<div class="notebook-code-input">
      ...
      if (cell.outputs?.length) {
        ...
        if (outputs) {
          content3 += `<div class="notebook-outputs">
    ...
    return `<div id="notebook-cell-${index2}" class="notebook-cell notebook-${escapeHtml(cell.cell_type)}-cell">${content3}</div>`;
  };
  const faviconDataUri = async (host) => {
    const cachePath = default2.join(opts.cacheDir, `favicon-${host}.txt`);
    try {
  ...
  const notebookToHtml = async (notebook, sourceUrl) => {
    const language = notebook.metadata?.language_info?.name ?? notebook.metadata?.kernelspec?.language ?? "python";
    const cells2 = (await Promise.all(notebook.cells.map((cell, i) => cellToHtml(cell, i, language)))).join("\n");
    ...
    ) : host ? `<span class="notebook-host">${escapeHtml(host)}</span>` : ""}
        </div>
      </div>
      <div class="notebook-content"><div class="notebook-cells">${cells2}</div></div>`;
  };
  const cachePathFor = (url) => default2.join(opts.cacheDir, `${Buffer.from(url).toString("base64url")}.json`);
  const readCache = async (url) => {
    try {
      return JSON.parse(await fs.readFile(cachePathFor(url), "utf-8"));
    } catch {
      return null;
    }
  };
  const writeCache = async (url, data) => {
    try {
  ...
  const readLocalNotebook = async (relativePath) => {
    const absolutePath = default2.resolve(process.cwd(), relativePath);
    try {
  ...
  const githubBlobUrl = (relativePath) => opts.repoUrl ? `${opts.repoUrl.replace(/\/+$/, "")}/blob/${opts.repoRef}/${relativePath}` : null;
  const download = async (url) => {
    const rawUrl = url.includes("github.com") && !url.includes("raw.githubusercontent.com") ? url.replace("github.com", "raw.githubusercontent.com").replace("/blob/", "/") : url;
    try {
        ...
        () => {
          return async (tree, _file) => {
            ...
            visit(tree, "element", (node2) => {
              ...
              if (node2.tagName === "a" && typeof href === "string" && href.endsWith(".ipynb")) {
                links.push(node2);
              }
            });
            if (links.length === 0) return;
            await Promise.all(
              links.map(async (node2) => {
                const href = node2.properties.href;
                try {
                  if (!/^https?:\/\//.test(href)) {
                    ...
                    const html8 = await notebookToHtml(notebook2, blobUrl ?? href);
                    node2.tagName = "div";
                    node2.properties = {
                      className: ["jupyter-notebook-embedded"],
                      "data-notebook-url": blobUrl ?? href
                    };
                    node2.children = fromHtml(html8, { fragment: true }).children;
                    return;
                  }
                  ...
                  const html7 = await notebookToHtml(notebook, href);
                  node2.tagName = "div";
                  node2.properties = {
                    className: ["jupyter-notebook-embedded"],
                    "data-notebook-url": href
                  };
                  node2.children = fromHtml(html7, { fragment: true }).children;
                } catch (error) {
...
var src_default = NotebookEmbedding;

export { NotebookEmbedding, src_default as default };
